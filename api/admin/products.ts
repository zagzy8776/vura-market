...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const beforeRows = await sql`SELECT id, name, price_kobo, source_price_kobo, stock_status, is_active, supplier_id, source_location FROM products WHERE id = ${productId} LIMIT 1`;
    if (!beforeRows[0]) return json(res, 404, { error: 'Product not found.' });
