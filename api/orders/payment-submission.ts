...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (typeof orderId !== 'string' || typeof transferReference !== 'string' || transferReference.trim().length < 3 || transferReference.trim().length > 100) {
      return json(res, 400, { error: 'Enter the transfer reference from your bank receipt.' });
    }

    const rows = await sql`
      UPDATE orders
      SET payment_status = 'pending_verification', status = 'payment_verification', transfer_reference = ${transferReference.trim()}, payment_submitted_at = now(), updated_at = now()
      WHERE id = ${orderId} AND buyer_id = ${user.id} AND payment_status = 'unpaid'
    ...
    await notifyAdmins({
      ...
      body: `A customer submitted transfer reference ${transferReference.trim()}.`,
