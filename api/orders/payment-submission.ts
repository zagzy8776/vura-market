...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const rows = await sql`UPDATE orders SET payment_status = 'pending_verification', status = 'payment_verification', transfer_reference = ${transferReference.trim()}, payment_submitted_at = now(), updated_at = now() WHERE id = ${orderId} AND payment_status = 'unpaid' RETURNING id, order_number, total_kobo, buyer_id`;
    ...
    if (!buyer) return json(res, 404, { error: 'Buyer not found for this order.' });

    const email = simpleOrderEmail(`Payment submitted for ${rows[0].order_number}`, buyer.name, rows[0].order_number, 'We received your transfer reference and will verify your payment before sourcing your product.');
    await notifyUser({ userId: rows[0].buyer_id, email: buyer.email, firstName: buyer.name, orderId: rows[0].id, eventType: 'payment.submitted', title: 'Payment submitted', body: 'Your transfer reference has been submitted. We will verify your payment before sourcing your order.', subject: email.subject, text: email.text, html: email.html });
