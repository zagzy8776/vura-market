...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (typeof name !== 'string' || name.trim().length < 2) return json(res, 400, { error: 'Category name is required.' });
    const slug = name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    const rows = await sql`INSERT INTO categories (name, slug, icon) VALUES (${name.trim()}, ${slug}, ${icon}) ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name RETURNING id, name, slug, icon`;
