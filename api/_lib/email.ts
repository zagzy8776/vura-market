...
export async function sendTransactionalEmail(event: EmailEvent) {
  const existing = event.orderId
    ? await sql`SELECT id FROM email_deliveries WHERE order_id = ${event.orderId} AND event_type = ${event.eventType} AND recipient = ${event.recipient} LIMIT 1`
    : [];
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from, to: [event.recipient], subject: event.subject, text: event.text, html: event.html }),
    });
...
export function orderEmail(order: { orderNumber: string; productName: string; totalKobo: number; accountNumber: string; accountName: string; bankName: string }, firstName: string) {
  ...
  return {
    ...
    text: `Hi ${firstName || 'there'}, your Vura order ${order.orderNumber} for ${order.productName} has been received. Total: ${amount}. Please transfer to ${order.accountName}, ${order.bankName}, account ${order.accountNumber}. Then confirm your transfer in My Orders.`,
...
export function simpleOrderEmail(subject: string, firstName: string, orderNumber: string, message: string) {
  ...
  return {
    subject,
    text: `Hi ${firstName || 'there'}, ${message} Order: ${orderNumber}.`,
