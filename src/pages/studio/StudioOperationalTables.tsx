...
import { ChevronRight, Package, Pencil, Plus, RefreshCw, X } from 'lucide-react';
...
export function OperationalProducts({ products, suppliers = [], onRefresh }: { products: Any[]; suppliers?: Any[]; onRefresh: () => void }) {
  ...
  return <section>
    <Toolbar title="Products" action={<div className="flex gap-2"><input value={q} onChange={e => setQ(e.target.value)} placeholder="Search products…" className={field.replace('mt-1 ','')}/><button onClick={() => setCreating(true)} className="flex items-center gap-2 rounded-xl bg-vura-500 px-4 py-2 text-sm font-bold"><Plus size={15}/> Add product</button></div>}/>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{rows.map(p => <article key={p.id} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[.02]"><div className="aspect-[4/3] bg-white/5">{p.images?.[0] ? <img src={p.images[0]} alt={p.name} className="h-full w-full object-cover"/> : <div className="grid h-full place-items-center text-white/25"><Package size={30}/></div>}</div><div className="p-4"><div className="font-bold">{p.name}</div><div className="text-xs text-white/35">{p.brand || '—'} · {p.category || 'Uncategorised'}</div><div className="mt-3 flex items-center justify-between"><b>{money(p.price_kobo)}</b><Status value={p.stock_status}/></div><button onClick={() => setSelected(p)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg border border-white/10 py-2 text-xs font-bold hover:bg-white/5"><Pencil size={13}/> Edit product</button></div></article>)}</div>
    {!rows.length && <Empty text="No products match your search."/>}
...
function ProductModal({ product, suppliers, onClose, onSaved }: { product: Any | null; suppliers: Any[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={product ? 'Edit product' : 'Add product'} onClose={onClose}>
    <div className="grid gap-4 md:grid-cols-2">
      <Label text="Product name"><input value={form.name} onChange={e=>set('name',e.target.value)} className={field}/></Label>
      <Label text="Brand"><input value={form.brand} onChange={e=>set('brand',e.target.value)} className={field}/></Label>
      ...
      <Label text="Category"><select value={form.categoryId} onChange={e=>set('categoryId',e.target.value)} className={field}><option value="">Uncategorised</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></Label>
      ...
      <Label text="Condition"><input value={form.condition} onChange={e=>set('condition',e.target.value)} className={field}/></Label>
      <Label text="Storage"><input value={form.storage} onChange={e=>set('storage',e.target.value)} className={field}/></Label>
      <Label text="Color"><input value={form.color} onChange={e=>set('color',e.target.value)} className={field}/></Label>
      <Label text="Source location"><input value={form.sourceLocation} onChange={e=>set('sourceLocation',e.target.value)} className={field}/></Label>
      <Label text="Description"><textarea value={form.description} onChange={e=>set('description',e.target.value)} rows={3} className={field}/></Label>
    </div>
    ...
    <div className="mt-6 flex justify-end gap-2"><button onClick={onClose} className="rounded-xl border border-white/10 px-4 py-2 text-sm">Cancel</button><button disabled={busy} onClick={() => void save()} className="rounded-xl bg-vura-500 px-5 py-2 text-sm font-bold disabled:opacity-50">{busy?'Saving…':product?'Save changes':'Create product'}</button></div>
  </Modal>;
}
...
function Modal({title,onClose,children}:{title:string;onClose:()=>void;children:ReactNode}){return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/70 p-0 backdrop-blur-sm md:items-center md:p-6"><div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-t-3xl border border-white/10 bg-[#0b0d17] p-6 shadow-2xl md:rounded-3xl"><div className="flex items-center justify-between"><h3 className="text-xl font-black">{title}</h3><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-lg border border-white/10"><X size={16}/></button></div><div className="mt-6">{children}</div></div></div>}
