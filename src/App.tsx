...
import {
  ArrowLeft, ArrowRight, Bell, BookOpen, Camera, Check, ChevronDown, ChevronRight,
  CreditCard, ExternalLink, Filter, Gamepad2, Headphones, Heart, Home, Laptop,
  LayoutGrid, LogOut, MapPin, Menu, Package, Plus, Search, Settings, ShoppingBag,
  Smartphone, Sparkles, Store, Truck, UserRound, Watch, X, Zap,
} from 'lucide-react';
import { useAuth, type AppUser } from '@/context/AuthContext';

type Category = { id: string; name: string; slug: string; icon: string };
type Product = {
  id: string; name: string; brand: string; category?: string; category_id?: string; description: string;
  price_kobo: number; condition_label: string; storage?: string | null; color?: string | null;
  stock_status: string; images: string[];
};
type Order = {
  id: string; order_number?: string; quantity: number; total_kobo: number; status: string;
  payment_status: string; sourcing_status?: string; delivery_name: string; delivery_phone: string;
  delivery_address: string; delivery_city: string; created_at: string; product_name: string;
  brand: string; images: string[]; transfer_reference?: string | null;
};
type View = 'home' | 'catalog' | 'deals' | 'product';
...
const fallbackCategories: Category[] = [
  ...
  { id: '', name: 'Monitors', slug: 'monitors', icon: 'Monitor' },
  { id: '', name: 'Tablets', slug: 'tablets', icon: 'Tablet' },
];
...
export default function App() {
  ...
  const openProduct = (product: Product) => { setSelected(product); setView('product'); window.scrollTo({ top: 0 }); };
  const go = (next: View) => { setView(next); setSelected(null); setMenuOpen(false); window.scrollTo({ top: 0 }); };
...
function Loading() {
  return <div className="grid min-h-screen place-items-center bg-white"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#5b35f5] text-white"><Zap size={19} fill="currentColor" /></span><span className="text-2xl font-black tracking-[-.07em]">VURA<span className="text-[#5b35f5]">.</span></span></div></div>;
}
    ...
    <div className="mx-auto max-w-[1440px] px-5 md:px-8">
      <div className="flex h-[70px] items-center gap-4">
        <button onClick={() => onView('home')} className="flex shrink-0 items-center gap-2.5">
          <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#5b35f5] text-white shadow-lg shadow-[#5b35f5]/20"><Zap size={20} fill="currentColor" /></span>
          <span className="text-[23px] font-black tracking-[-.08em]">VURA<span className="text-[#5b35f5]">.</span></span>
        </button>
        <div className="hidden min-w-0 flex-1 md:block"><div className="mx-auto max-w-xl"><div className="relative"><Search size={17} className="absolute left-4 top-3.5 text-[#98a0b3]"/><input value={search} onChange={e => onSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && onView('catalog')} placeholder="Search phones, laptops, gaming, accessories..." className="w-full rounded-xl border border-[#e2e5ee] bg-[#fafbfe] py-3 pl-11 pr-12 text-sm outline-none transition focus:border-[#5b35f5] focus:bg-white"/><button onClick={() => onView('catalog')} className="absolute right-1.5 top-1.5 grid h-10 w-10 place-items-center rounded-lg bg-[#5b35f5] text-white"><Search size={16}/></button></div></div></div>
        <nav className="hidden items-center gap-6 text-sm font-semibold text-[#5f6678] lg:flex">
          <button onClick={() => onView('catalog')} className={view === 'catalog' ? 'text-[#5b35f5]' : ''}>Products</button>
          <button onClick={() => onView('deals')} className={view === 'deals' ? 'text-[#5b35f5]' : ''}>Deals</button>
          <button onClick={() => onView('home')} className="text-[#5f6678]">New arrivals</button>
          {user?.role === 'admin' && (
            <a href="/studio" className="text-[#5b35f5] font-bold">Studio</a>
          )}
        </nav>
...
function HomePage({ categories, products, onView, onProduct }: { categories: Category[]; products: Product[]; onView: (v: View) => void; onProduct: (p: Product) => void }) {
  ...
  return <main>
    ...
    <section className="mx-auto max-w-[1440px] px-5 py-12 md:px-8"><div className="flex items-end justify-between"><div><p className="text-sm font-bold text-[#ef7b3a]">DEALS</p><h2 className="mt-1 text-3xl font-black tracking-[-.05em]">Good prices, no noise.</h2></div><button onClick={() => onView('deals')} className="hidden text-sm font-bold text-[#5b35f5] sm:flex items-center gap-1">View deals <ArrowRight size={16}/></button></div><div className="mt-7 grid gap-5 md:grid-cols-2"><DealCard title="Gaming weekend" text="Consoles, controllers and accessories." icon={<Gamepad2 size={34}/>} /><DealCard title="Workday upgrade" text="Laptops, monitors and essentials." icon={<Laptop size={34}/>} /></div></section>
...
function TrustStrip() { return <div className="border-b border-[#e8eaf1] bg-white"><div className="mx-auto grid max-w-[1440px] grid-cols-2 md:grid-cols-4">{[['Quality products', ShoppingBag], ['Secure bank transfer', CreditCard], ['Fast local delivery', Truck], ['Support when needed', BookOpen]].map(([label, Icon]) => <div key={String(label)} className="flex items-center gap-3 border-r border-[#eef0f5] px-5 py-4 last:border-r-0"><span className="grid h-9 w-9 place-items-center rounded-lg bg-[#f2f0ff] text-[#5b35f5]"><Icon size={17}/></span><span className="text-sm font-semibold text-[#5d6577]">{String(label)}</span></div>)}</div></div>; }

function SectionHeading({ eyebrow, title, action, onAction }: { eyebrow: string; title: string; action: string; onAction: () => void }) { return <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#8b93a5]">{eyebrow}</p><h2 className="mt-1 text-2xl font-black tracking-[-.04em] md:text-3xl">{title}</h2></div><button onClick={onAction} className="flex shrink-0 items-center gap-1 text-sm font-bold text-[#5b35f5]">{action}<ArrowRight size={15}/></button></div>; }
...
function Catalog({ products, categories, search, onSearch, onProduct, deals = false }: { products: Product[]; categories: Category[]; search: string; onSearch: (v: string) => void; onProduct: (p: Product) => void; deals?: boolean }) {
  ...
  return <main className="mx-auto max-w-[1440px] px-5 py-8 md:px-8"><div className="mb-6 flex items-center gap-2 text-sm text-[#9299aa]"><button className="hover:text-[#5b35f5]">Home</button><ChevronRight size={14}/><span>{deals ? 'Deals' : 'Products'}</span></div><div className="flex flex-col gap-4 border-b border-[#e7e9f0] pb-6 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#8f97a9]">{deals ? 'Special pricing' : 'Marketplace'}</p><h1 className="mt-1 text-4xl font-black tracking-[-.055em]">{deals ? 'Deals worth checking.' : 'All products.'}</h1></div><div className="relative w-full md:w-96"><Search size={17} className="absolute left-3.5 top-3.5 text-[#98a0b2]"/><input value={search} onChange={e => onSearch(e.target.value)} placeholder="Search the catalog..." className="w-full border border-[#dfe2ea] bg-white py-3 pl-10 pr-4 text-sm outline-none focus:border-[#5b35f5]"/></div></div><div className="mt-6 grid gap-8 lg:grid-cols-[230px_1fr]"><aside className="hidden lg:block"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#939aac]">Categories</p><div className="mt-3 space-y-1">{['All', ...categories.map(c => c.name)].map(c => <button key={c} onClick={() => setCategory(c)} className={`flex w-full items-center justify-between px-3 py-2.5 text-left text-sm font-semibold ${category === c ? 'bg-[#f0edff] text-[#5b35f5]' : 'text-[#60687a] hover:bg-[#f7f8fb]'}`}><span>{c}</span>{category === c && <Check size={15}/>}</button>)}</div><div className="mt-8 border-t border-[#e8eaf1] pt-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#939aac]">Trust</p><div className="mt-3 space-y-3 text-sm text-[#727b8c]"><p className="flex items-center gap-2"><Package size={15} className="text-[#5b35f5]"/> In stock</p><p className="flex items-center gap-2"><Truck size={15} className="text-[#5b35f5]"/> Delivery available</p></div></div></aside><div><div className="mb-4 flex items-center justify-between"><div className="flex gap-2 overflow-x-auto lg:hidden">{['All', ...categories.slice(0, 6).map(c => c.name)].map(c => <button key={c} onClick={() => setCategory(c)} className={`shrink-0 px-4 py-2.5 text-sm font-bold ${category === c ? 'bg-[#17182a] text-white' : 'bg-[#f0f1f6] text-[#677083]'}`}>{c}</button>)}</div><span className="text-xs font-semibold text-[#9299aa]">{filtered.length} products</span></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{filtered.length ? filtered.map(p => <ProductCard key={p.id} product={p} onClick={() => onProduct(p)} />) : <div className="col-span-full border border-dashed border-[#dfe2e9] py-24 text-center"><Package className="mx-auto text-[#b6bdc9]" size={38}/><h3 className="mt-4 text-xl font-black">Nothing here yet</h3><p className="mt-2 text-sm text-[#8a92a4]">We're preparing fresh Vura listings.</p></div>}</div></div></div></main>;
}
...
function PaymentModal({ order, payment, onClose }: { order: Order; payment: { accountNumber: string; accountName: string; bankName: string }; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[70] grid place-items-end bg-black/45 p-0 md:place-items-center md:p-6"><div className="w-full max-w-xl border border-[#dfe2ea] bg-white p-6 shadow-2xl md:p-8"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#5b35f5]">Order {order.order_number}</p><h2 className="mt-1 text-2xl font-black">Complete your bank transfer</h2><p className="mt-2 text-sm text-[#7d8596]">Send the exact order total to the Vura account below.</p></div><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-lg hover:bg-[#f5f6fa]"><X size={18}/></button></div><div className="mt-6 border border-[#dcdff0] bg-[#f6f3ff] p-5"><p className="text-xs font-bold uppercase tracking-[.13em] text-[#737b8d]">Bank transfer</p><div className="mt-4 grid gap-3 text-sm"><Row label="Bank" value={payment.bankName}/><Row label="Account name" value={payment.accountName}/><Row label="Account number" value={payment.accountNumber}/><Row label="Amount" value={money(order.total_kobo)}/></div></div><div className="mt-6"><label className="text-sm font-bold">Transfer reference</label><input value={ref} onChange={e => setRef(e.target.value)} placeholder="Enter the reference from your bank" className="mt-2 w-full border border-[#dfe2ea] px-4 py-3 outline-none focus:border-[#5b35f5]"/><button onClick={submit} disabled={busy} className="mt-3 w-full rounded-xl bg-[#17182a] px-4 py-3 font-bold text-white disabled:opacity-60">{busy ? 'Submitting...' : 'I have made the transfer'}</button>{msg && <p className="mt-3 text-sm font-semibold text-[#4d6b57]">{msg}</p>}</div><p className="mt-5 text-xs leading-5 text-[#9aa1b0]">Your payment is marked for verification until Vura confirms the transfer. Supplier pricing and sourcing information are never shown here.</p></div></div>;
}
...
function DealCard({ title, text, icon }: { title: string; text: string; icon: React.ReactNode }) { return <div className="flex items-center justify-between overflow-hidden border border-[#e2e4eb] bg-white p-6 md:p-8"><div><div className="grid h-12 w-12 place-items-center bg-[#f0edff] text-[#5b35f5]">{icon}</div><h3 className="mt-5 text-2xl font-black">{title}</h3><p className="mt-2 text-sm text-[#7b8394]">{text}</p></div><ExternalLink className="text-[#9aa1b2]" size={26}/></div>; }
